...
def time_features(y):
    ...
    kurt = stats.kurtosis(y, fisher=True, bias=False, nan_policy='propagate')
    zcr = ((y[:-1]*y[1:]) < 0).mean()
...
def welch_features(y, fs=FS):
    ...
    psd_sum = np.sum(Pxx)
    centroid = np.sum(f * Pxx) / (psd_sum + 1e-20)
    bw = np.sqrt(np.sum(((f - centroid)**2) * Pxx) / (psd_sum + 1e-20))
...
def band_energy_features(y, fs=FS):
    ...
    for i, (fl, fh) in enumerate(bands, 1):
        mask = (f >= fl) & (f < fh)
        feats[f"band{i}_ratio"] = np.sum(Pxx[mask]) / (total + 1e-20)
    return feats
...
nan_cols = np.isnan(X_all).any(axis=0)
inf_cols = ~np.isfinite(X_all).all(axis=0)
const_cols = np.nanstd(X_all, axis=0) < 1e-15
if nan_cols.any() or inf_cols.any() or const_cols.any():
    bad = []
    for i, name in enumerate(feature_cols):
        flags = []
        if nan_cols[i]: flags.append("NaN")
        if inf_cols[i]: flags.append("Inf")
        if const_cols[i]: flags.append("Const")
        if flags:
            bad.append(f"{name}({','.join(flags)})")
    print("[Feature Check] Suspicious columns ->", ", ".join(bad) if bad else "None")
...
X_P, y_P = known_P[feature_cols].to_numpy(dtype=float), known_P["P"].to_numpy(dtype=float)
X_V, y_V = known_V[feature_cols].to_numpy(dtype=float), known_V["V"].to_numpy(dtype=float)
...
model_P = HistGradientBoostingRegressor(**common_params)
model_V = HistGradientBoostingRegressor(**common_params)
...
pred_rows = []
if not need_P.empty:
    Xp = need_P[feature_cols].to_numpy(dtype=float)
...
if not need_V.empty:
    Xv = need_V[feature_cols].to_numpy(dtype=float)
...
def permutation_importance_bar(estimator, X, y, feature_names, fname, topk=15):
    ...
    imp = np.abs(result.importances_mean)
...
permutation_importance_bar(
    model_P, X_P, y_P, feature_cols,
    os.path.join(RESULTS_DIR, "feat_imp_P.png")
)
permutation_importance_bar(
    model_V, X_V, y_V, feature_cols,
